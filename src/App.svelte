...
<div class="flex h-screen w-screen overflow-hidden bg-background text-on-background select-none">
  <!-- Sidebar -->
  <Sidebar />
...
{#if store.confirmDialog}
  <div class="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fade-in select-none">
    <div class="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 w-[360px] shadow-2xl flex flex-col gap-4">
      <div class="flex items-center gap-3 text-error">
        <span class="material-symbols-outlined text-2xl">warning</span>
        <h3 class="font-bold text-base text-on-surface">{store.confirmDialog.title}</h3>
      </div>
