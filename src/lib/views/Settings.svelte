...
<main class="grow overflow-hidden bg-surface-container-lowest flex h-full">
  ...
  <div class="w-64 shrink-0 bg-surface-container-low border-r border-outline-variant flex flex-col justify-between p-6">
    ...
    <div class="flex flex-col items-center justify-center gap-2 pt-6 border-t border-outline-variant/30">
      <div class="flex items-center justify-center gap-1.5 opacity-40 select-none text-[9px] font-bold uppercase tracking-wider text-on-surface-variant">
        <span class="material-symbols-outlined text-[10px]">info</span>
        <span>Version {pkg.version}</span>
      </div>
      <button 
        onclick={handleCopyGithubLink}
        class="flex items-center justify-center gap-1.5 opacity-40 hover:opacity-75 transition-opacity text-[9px] font-bold uppercase tracking-wider text-on-surface-variant cursor-pointer focus:outline-none border-0 bg-transparent p-0"
      >
